...
const FormerBgSection = memo(function FormerBgSection() {
  const {
    formerBgRemoved, isRemovingFormerBg, formerBgTolerance, formerLineColor,
    isPickingColor, pickingTarget,
    toggleFormerBgRemoval, setFormerBgTolerance, setFormerLineColor,
    startColorPick, cancelColorPick,
  } = usePantagraphStore(
    useShallow((s) => ({
  ...
  const isPickingFormer = isPickingColor && pickingTarget === 'former';
    ...
    <div className="space-y-2 py-2">
      ...
      {formerBgRemoved && (
        <div className="space-y-1 pt-1">
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-muted-foreground">টলারেন্স</span>
            <span className="text-[10px] font-mono text-muted-foreground">{formerBgTolerance}</span>
          </div>
          <input
          ...
          <div className="flex items-center gap-1.5">
            {[
              ...
              <button
                key={value}
                type="button"
                onClick={() => setFormerLineColor(value)}
                className={`px-2 py-0.5 rounded text-[10px] font-medium border transition-all ${cls}`}
              >
                {label}
              </button>
            ))}
...
const CurrentBgSection = memo(function CurrentBgSection() {
  const {
    currentBgRemoved, isRemovingCurrentBg, currentBgTolerance, currentLineColor,
    isPickingColor, pickingTarget,
    toggleCurrentBgRemoval, setCurrentBgTolerance, setCurrentLineColor,
    startColorPick, cancelColorPick,
  } = usePantagraphStore(
    useShallow((s) => ({
  ...
  const isPickingCurrent = isPickingColor && pickingTarget === 'current';
    ...
    <div className="space-y-2 py-2">
      ...
      {currentBgRemoved && (
        <div className="space-y-1 pt-1">
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-muted-foreground">টলারেন্স</span>
            <span className="text-[10px] font-mono text-muted-foreground">{currentBgTolerance}</span>
          </div>
          <input
            type="range" min="0" max="255"
          ...
          <div className="flex items-center gap-1.5">
            {[
              ...
              <button
                key={value}
                type="button"
                onClick={() => setCurrentLineColor(value)}
                className={`px-2 py-0.5 rounded text-[10px] font-medium border transition-all ${cls}`}
              >
                {label}
              </button>
  ...
  return (
    <div className="space-y-3 my-4">
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between">
          <Label className="text-xs text-muted-foreground flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-destructive" />সাবেক
          </Label>
          <span className="text-[10px] font-mono text-muted-foreground">{Math.round(formerOpacity * 100)}%</span>
        </div>
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between">
          <Label className="text-xs text-muted-foreground flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-primary" />হাল
          </Label>
